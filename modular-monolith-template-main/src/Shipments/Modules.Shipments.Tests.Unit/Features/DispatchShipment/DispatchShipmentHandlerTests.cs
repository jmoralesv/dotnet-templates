...
using Modules.Shipments.Features.Features.DispatchShipment;
using Modules.Shipments.Infrastructure.Database;
using NSubstitute;

namespace Modules.Shipments.Tests.Unit.Features.DispatchShipment;

public class DispatchShipmentHandlerTests : IAsyncDisposable
{
    ...
    public DispatchShipmentHandlerTests()
    {
        ...
        _logger = loggerFactory.CreateLogger<DispatchShipmentHandler>();

        _handler = new DispatchShipmentHandler(_dbContext, _logger);
    }
    ...
    [Fact]
    public async Task DispatchShipmentHandler_ShouldDispatchShipment_WhenShipmentExists()
    {
        // Arrange
        ...
        Assert.Equal(ShipmentStatus.Dispatched, updatedShipment.Status);
    }

    [Fact]
    public async Task DispatchShipmentHandler_ShouldReturnError_WhenShipmentDoesNotExist()
    {
        // Arrange
    ...
    [Fact]
    public async Task DispatchShipmentHandler_ShouldReturnError_WhenShipmentCannotBeDispatched()
    {
        // Arrange
        var shipment = CreateTestShipment();
