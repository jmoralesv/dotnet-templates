...
namespace Modules.Shipments.Tests.Integration.Tests.TransitShipment;

[Collection("ShippingTests")]
public class TransitShipmentTests(CustomWebApplicationFactory webFactory)
	: BaseTest(webFactory), IAsyncLifetime
{
	...
	[Fact]
	public async Task TransitShipment_ShouldTransitShipment_WhenShipmentExists()
	{
		// Arrange
		...
		var httpResponse = await WebFactory.HttpClient.PostAsync($"/api/shipments/transit/{shipment.Number}", null);
	...
	[Fact]
	public async Task TransitShipment_ShouldReturnError_WhenShipmentDoesNotExist()
	{
		// Arrange
		// Act
		var httpResponse = await WebFactory.HttpClient.PostAsync("/api/shipments/transit/12345", null);
	...
	[Fact]
	public async Task TransitShipment_ShouldReturnError_WhenShipmentCannotBeTransited()
	{
		// Arrange
		var address = new AddressResponse("Amazing st. 5", "New York", "127675");
		...
		Assert.Equal($"Can only update to InTransit from Dispatched status for shipment {shipment.Number}", error.Value.First());
	}
}
