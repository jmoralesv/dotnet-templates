...
namespace Modules.Shipments.Tests.Integration.Tests.ReceiveShipment;

[Collection("ShippingTests")]
public class ReceiveShipmentTests(CustomWebApplicationFactory webFactory)
	: BaseTest(webFactory), IAsyncLifetime
{
	...
	[Fact]
	public async Task ReceiveShipment_ShouldReceiveShipment_WhenShipmentExists()
	{
		// Arrange
		...
		await WebFactory.HttpClient.PostAsync($"/api/shipments/deliver/{shipment.Number}", null);
		var httpResponse = await WebFactory.HttpClient.PostAsync($"/api/shipments/receive/{shipment.Number}", null);
	...
	[Fact]
	public async Task ReceiveShipment_ShouldReturnError_WhenShipmentDoesNotExist()
	{
		// Arrange
		// Act
		var httpResponse = await WebFactory.HttpClient.PostAsync("/api/shipments/receive/12345", null);
	...
	[Fact]
	public async Task ReceiveShipment_ShouldReturnError_WhenShipmentCannotBeReceived()
	{
		// Arrange
		var address = new AddressResponse("Amazing st. 5", "New York", "127675");
		...
		Assert.Equal($"Can only update to Received from Delivered status for shipment {shipment.Number}", error.Value.First());
	}
}
