...
namespace Modules.Shipments.Tests.Integration.Tests.CancelShipment;

[Collection("ShippingTests")]
public class CancelShipmentTests(CustomWebApplicationFactory webFactory)
	: BaseTest(webFactory), IAsyncLifetime
{
	...
	[Fact]
	public async Task CancelShipment_ShouldCancelShipment_WhenShipmentExists()
	{
		// Arrange
		...
		var httpResponse = await WebFactory.HttpClient.PostAsync($"/api/shipments/cancel/{shipment.Number}", null);
	...
	[Fact]
	public async Task CancelShipment_ShouldReturnError_WhenShipmentDoesNotExist()
	{
		// Arrange
		// Act
		var httpResponse = await WebFactory.HttpClient.PostAsync("/api/shipments/cancel/12345", null);
	...
	[Fact]
	public async Task CreateShipment_ShouldReturnError_WhenShipmentCannotBeCancelled()
	{
		// Arrange
		var address = new AddressResponse("Amazing st. 5", "New York", "127675");
