...
namespace Modules.Shipments.Tests.Integration.Tests.ProcessShipment;

[Collection("ShippingTests")]
public class ProcessShipmentTests(CustomWebApplicationFactory webFactory)
	: BaseTest(webFactory), IAsyncLifetime
{
	...
	[Fact]
	public async Task ProcessShipment_ShouldProcessShipment_WhenShipmentExists()
	{
		// Arrange
		...
		var httpResponse = await WebFactory.HttpClient.PostAsync($"/api/shipments/process/{shipment.Number}", null);
	...
	[Fact]
	public async Task ProcessShipment_ShouldReturnError_WhenShipmentDoesNotExist()
	{
		// Arrange
		// Act
		var httpResponse = await WebFactory.HttpClient.PostAsync("/api/shipments/process/12345", null);
	...
	[Fact]
	public async Task ProcessShipment_ShouldReturnError_WhenShipmentCannotBeProcessed()
	{
		// Arrange
		var address = new AddressResponse("Amazing st. 5", "New York", "127675");
		...
		Assert.Equal($"Can only update to Processing from Created status for shipment {shipment.Number}", error.Value.First());
	}
}
